...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        if (direction == RIGHT)
            Position += Right * velocity;
        if (direction == UP)
            Position += WorldUp * velocity; // added for raising camera (Q) 
        if (direction == DOWN)
            Position -= WorldUp * velocity; // added for lowering camer (E)
    }
    ...
    {
        float const modifier= 0.02; // variable to adjust movement speed

        if (yoffset > 0) { // increase speed if wheel moves in postive direction
            MovementSpeed += yoffset + modifier;
            if (MovementSpeed > 7)
                MovementSpeed = 7; // speed cap
        }

        if (yoffset < 0) { // decrease speed if wheel moves in negative direction
            MovementSpeed -= MovementSpeed - modifier;
            if (MovementSpeed < 1)
                MovementSpeed = 1; // boundary to keep object from moving in oppsite direction at 0
        }
